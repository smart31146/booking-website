...
If  Request.querystring("quiz") <> "" Then
    ...
    ' Check if SessionID existx
		...
		response.redirect "t_feedback.asp?question_ID="& request.querystring("question_ID") &"&amp;SessionBBP="& Session("sessionID")  &"&currentID="& request.querystring("currentID") &""
...
function trySubmit(){
    if (empty == true)
    {
        alert("You must make a selection before moving on.\nPlease click 'OK' and then select an answer before hitting 'Submit'.");
        return false;
    }
...
{
if (b_click){alert('Please click on an information box to proceed.'); return} else {self.location=wheretogo}
}
...
<div style="float:right;padding:8px 8px 15px 20px;">
            ...
            <div class="clear"></div>
              ...
              <% ' If last page on traning & quiz
                ...
                <script type="text/javascript">
                    window.setTimeout("document.location = 'certificate.asp?SessionBBP=<%=Session("sessionID")%>';", 5000);
					
                </script>
                
            <div class="div_button">
                <div class="next_blue" style="width:160px;">
                    <div class="h_submit_blue"><a href="certificate.asp?SessionBBP=<% =Session("sessionID")%>" class="box_link" style="padding-left:15px;">CERTIFICATE</a></div>
                </div>
            </div>
                <% ELSE%>
                ...
                <% IF clng(prevID)<>0 And subject("s_order") - previousTopicEnd <> 1 THen%>
                    <div class="back_blue">
                        <div class="h_submit_blue"><a href="t_question.asp?nextID=<% =prevID%>&returning=1&amp;SessionBBP=<% =Session("sessionID")%>" class="box_link" style="padding-left:65px;padding-top:5px;">BACK</a></div>
                    </div>
                <% end if%>
                    <div class="next_blue">
                <% IF showQuestion = true THEN%>
                    <div class="h_submit_blue" ><a href="javascript:gotonextpage('t_question.asp?nextID=<% =nextID%>&amp;SessionBBP=<% =Session("sessionID")%>')" onClick="closing=false" class="box_link" style="padding-left:15px;padding-top:5px;">NEXT</a></div>
                <% ELSE%>
                    <div class="h_submit_blue" ><a href="t_question.asp?nextID=<% =nextID%>&amp;SessionBBP=<% =Session("sessionID")%>" class="box_link" style="padding-left:15px;padding-top:5px;">NEXT</a></div>
                <% END IF%>
                </div>
            <% END IF %>
            </div>
                        ...
                        <% 
                         IF showQuestion = true THEN
                             For i=0 to ubound(QchoiceArr,2) %>
                                ...
                                <div style="margin:8px 0px 8px 0px;cursor:pointer;" >
									...
									<div class="quiz_choose_inside">
									<label style="cursor:pointer; display:block;" for="rbutton<% =QchoiceArr(0,i)%>"  ><% =ReplaceStrQuiz(QchoiceArr(2,i))%></label>
									</div>
								</div>
								</div>
                                </div>
                         ...
                         END IF%><br>
                            <div class="div_button" style="width:860px;text-align:right;margin:20px 0px;">
                                <input type="submit" style="background:url('images/button_submit.png') no-repeat;height:35px; border:none;width:130px;cursor:pointer;" name="btnSubmit" id="btnSubmit" value="">
                            </div>
                        </form>
                    </div>
                </div>
