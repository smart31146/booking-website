...
%>
	...
	<div class="header">
		...
		<ul class="menu">
		<% IF Session("userID") <> "" AND Session("id") <> "" THEN%>
			<li  style="float:left;margin-right:0px;" id="training"><a href="t_index.asp">TRAINING</a></li>
			...
			<!-- Updated code to remove user ID from URL by PR on23.02.2016-->
			<% IF Session("LMS") <> "1" THEN %>
			<li style="float:left;margin-left: -10px;width: 200px;" id="results">
		...
		<%END IF%>
			...
			<a style="padding-right:30px;" href="index.asp?alt=change" id="changesubject" >CHANGE SUBJECT</a>
			...
			IF Session("LMS") <> "1"  THEN 
			%> <li style="float:left;margin-left:-10px;"><a style="padding-left:15px;" href="change_password.asp" id="changepass" >CHANGE PASSWORD</a></li>
			<li style="float:right;margin-left:0px;" id="logout"><a style="padding-left:10px;" href="#" onclick="swal({   title: 'Do you want to logout?',   text: '',   type: 'warning',   showCancelButton: true,   confirmButtonColor: '#DD6B55',   confirmButtonText: 'Yes',   closeOnConfirm: false }, function(){  window.location.href='index.asp?alt=logout'; return false; });">LOG OUT</a></li>
			<% end if %>
				<% if curPageName = "g_index.asp" THEN %>
