...
if not obj.eof then
	...
	if obj("user_email")<>"" then
		...
		strMessage = strMessage + "<tr><td>Click on this link to reset your password <a href='https://www.lawofthejungle.com/"&client_name_short&"/reset_password.asp?uid="&URLEncode(StringToHex(all))&"'>Reset password</a></td></tr>"
...
Set obj = Server.CreateObject("ADODB.Recordset")
SQL="SELECT * FROM q_user WHERE upper(user_email)='"&UCASE(request("email_req"))&"'"
obj.ActiveConnection = Connect
obj.Source = SQL
obj.CursorType = 0
obj.CursorLocation = 3
