...
<%=vbcrlf%>"Field Feedback Report"

<%
Title_row = "ID, Date, Lodged By, Details"
%>


<%=vbcrlf%> <%=Title_row%> <%=vbcrlf%>
<%
While (NOT logs.EOF)

	Data_row = logs.Fields.Item("ID_feedback").Value & " , " & logs.Fields.Item("f_date").Value & " , " & logs.Fields.Item("user_firstname").Value & " " & logs.Fields.Item("user_lastname").Value & " , " & logs.Fields.Item("details").Value
 %>
	<%=Data_row%>
	<%=vbcrlf%>
	<%
...
<%=vbcrlf%><%=vbcrlf%>"Generated on:","<%=Now()%>"
<%=vbcrlf%>
Start date:, <%=fromdate%>
End date:, <%=todate%>
User name:, <%=username%>
Business Group:, <%if info1 <> 0 then%><%Response.Write info11.Fields.item("info1").value%><%info11.close%><%else%>All<%end if%>
Business Site:, <%if info2 <> 0 then%><%Response.Write info22("info2")%><%info22.close%><%else%>All<%end if%>
Business Activity:, <%if info3 <> 0 then%><%Response.Write info33("info3")%><%info33.close%><%else%>All<%end if%>
<%=vbcrlf%>
<%=vbcrlf%>"Copyright <% Response.Write Year(now) %> (c) Law of the Jungle Pty Limited"
<%
logs.Close()
%>
