    ...
    Function FindDuplicateUserName()
       ' boolean to abort record edit
        ...
        ' query string to execute
            ...
            ObjRS1.Open "SELECT * FROM q_user WHERE user_username='" & Replace(DuplicateUID,"'","''") & "'", MM_bbg_conn_STRING
            ...
            If Not ObjRS1.EOF Or Not ObjRS1.BOF Then
				...
				if(NOT ObjRS1("user_active")) then
				
				Set uobj = Server.CreateObject("ADODB.Command")
SQL="update q_user set user_active=1 WHERE user_username='" & Replace(DuplicateUID,"'","''") & "'"
uobj.ActiveConnection = MM_bbg_conn_STRING
uobj.CommandText = SQL
uobj.Execute
uobj.ActiveConnection.Close
Response.Write("<BR>User: <b>" & ObjRS1("user_username") & "</b> is set to active <BR>")
...
'***************************************************Function (2) Filling an Array with the info. of a user******************************
    Sub FillArrayWithUserInfo()
            ...
            MM_editConnection = MM_bbg_conn_STRING
            MM_editTable = "q_user"
            MM_editRedirectUrl = "q_list_of_users.asp"
            MM_fieldsStr  = "first_name|value|last_name|value|login_name|value|login_pass|value|info1|value|info2|value|info3|value|active|value|session|value|email|value|change_pass|value|reference|value"
           MM_columnsStr = "user_firstname|',none,''|user_lastname|',none,''|user_username|',none,''|user_city|',none,NULL|user_info1|none,none,NULL|user_info2|none,none,NULL|user_info3|none,none,NULL|user_active|none,1,0|user_new_session|',none,''|user_email|',none,''|user_password|none,1,0|user_reference|',none,''"
...
info11.ActiveConnection = MM_bbg_conn_STRING
...
info22.ActiveConnection = MM_bbg_conn_STRING
...
info33.ActiveConnection = MM_bbg_conn_STRING
...
Function GetSubjects()
    set subjects_b = Server.CreateObject("ADODB.Recordset")
			subjects_b.ActiveConnection = MM_bbg_conn_STRING
...
sub create_user_subjects
	...
	new_user.ActiveConnection = MM_bbg_conn_STRING
	...
	if request_send_user_email = 1 then
		...
		last_id_insert.ActiveConnection = MM_bbg_conn_STRING
	...
	For Each q in SplitSubjects
		...
		if (((InStr(q,"user_subject"))>0)=True) then
				...
				Set MM_editCmd = Server.CreateObject("ADODB.Command")
				MM_editCmd.ActiveConnection = MM_bbg_conn_STRING
				...
				if request_send_user_email = 1 then
					...
					check_subject_quiz.ActiveConnection = MM_bbg_conn_STRING
					...
					if (not check_subject_quiz.eof) then
				    	...
				    	MM_insertCmd.ActiveConnection = MM_bbg_conn_STRING
...
Function BusinessIsValid(Entry)
	...
	info1.ActiveConnection = MM_bbg_conn_STRING
...
Function SiteIsValid(Entry)
	...
	info2.ActiveConnection = MM_bbg_conn_STRING
 ...
 Function ActivityIsValid(Entry) 
	...
	info3.ActiveConnection = MM_bbg_conn_STRING
 ...
 End Function
  ...
  Function CompanyIsValid(Entry) 
	...
	info4.ActiveConnection = MM_bbg_conn_STRING
...
%>
 ...
 <%
   '***************************************************Main Body of the Program************************************************
        ...
        Path = "C:\www\BBP\BBP_acme_june2014\admin"
