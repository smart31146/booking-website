...
topics_stats.Source = "SELECT COUNT(logs.ID_log) AS ID_log, MAX(logs.log_date) AS log_date, b_topics.ID_topic, b_topics.topic_name FROM logs INNER JOIN (subjects INNER JOIN b_topics ON subjects.ID_subject = b_topics.topic_subject) ON logs.log_topicID = b_topics.ID_topic GROUP BY logs.log_module, subjects.ID_subject, b_topics.ID_topic, b_topics.topic_name HAVING (logs.log_module LIKE 'bbg') AND (subjects.ID_subject = " + Replace(subj, "'", "''") + ") ORDER BY COUNT(logs.ID_log) DESC;"
' SQL: "SELECT COUNT(logs.ID_log) AS ID_log, MAX(logs.log_date) AS log_date, b_topics.ID_topic, b_topics.topic_name  FROM b_topics INNER JOIN subjects ON b_topics.topic_subject = subjects.ID_subject RIGHT OUTER JOIN logs ON b_topics.ID_topic = logs.log_topicID  GROUP BY logs.log_module, subjects.ID_subject, b_topics.ID_topic, b_topics.topic_name  HAVING (logs.log_module LIKE 'bbg') AND (subjects.ID_subject = " + Replace(subj, "'", "''") + ")  ORDER BY COUNT(logs.ID_log) DESC;"
...
<TITLE>BBP ADMIN: Reference topics. You are logged in as <%=Session("MM_Username_admin")%></TITLE>
...
<%
call log_the_page ("BBG List Topics Stats: " & subj)
%>
