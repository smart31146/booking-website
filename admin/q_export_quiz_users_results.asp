...
<%
Response.Clear()
Response.AddHeader "Content-Disposition","attachment; filename=quiz_user_results" & day(now()) & "_" & month(now()) & "_" & year(now()) & ".csv"
Response.ContentType="application/vnd.ms-excel"
'Basic-ultradev DownloadRepeatRegion Server Behavior
%>"BBP - Quiz - session details"<%=vbcrlf%><%=vbcrlf%>""<%=(user.Fields.Item("user_firstname").Value)%>,<%=(user.Fields.Item("user_lastname").Value)%>'s session in <%=(sessions.Fields.Item("subject_name").Value)%>, <%=(sessions.Fields.Item("Session_date").Value)%>:"<%=vbcrlf%><%=vbcrlf%>"Question","Topic","Answer","Correct"<%=vbcrlf%>"-------------------------------------------------------------------------------------------------------------------------"<%=vbcrlf%><%
user_all = 0
user_correct = 0
While ((Repeat1__numRows <> 0) AND (NOT answers.EOF)) 
%>"<%=ClearHTMLTags(answers.Fields.Item("question_body").Value,2)%>","<%=(answers.Fields.Item("s_topic").Value)%>","<%=(answers.Fields.Item("choice_label").Value)%>","<%
