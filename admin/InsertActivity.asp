	...
	FindDuplicateActivity = ""
       ' boolean to abort record edit
        ...
        ' query string to execute
            ...
            DuplicateUID = user_activity
            set ObjRS1 = Server.CreateObject("ADODB.Recordset")
            ObjRS1.Open "SELECT * FROM q_info3 WHERE info3='" & Replace(DuplicateUID,"'","''") & "'", Connect
           
            'Activity already exsits in the data base
            If Not ObjRS1.EOF Or Not ObjRS1.BOF Then
			
                FindDuplicateBusiness = "True"
                Response.Write("<BR>Activity: <b>" & ObjRS1("info3") & "</b> already exists <br>")
			  BizID=ObjRS1("ID_info3") 
				
				else
				 Response.Write("<BR> New Activity")
				
				'Response.Write(ObjRS1("ID_info3"))
             End If
             ObjRS1.Close
             Set ObjRS1 = nothing
...
'Hereeeeeeeeeeeeeeeeeee Set Request_MM_insert = True when the submit button is pressed
  ...
  For i = LBound(MM_fields) To UBound(MM_fields) Step 2
    ...
    End if
	Response.Write("<BR><BR>Connecting to the file: <b>" & user_activity & "</b> ...<BR>")
    MM_tableValues = MM_tableValues & MM_columns(i)
    MM_dbValues = MM_dbValues & FormVal
	
  Next
 MM_editQuery = "insert into " & MM_editTable & " (" & MM_tableValues & ") values ('" & user_activity & "')"
