...
topics_stats.Source = "SELECT COUNT(logs.ID_log) AS ID_log, MAX(logs.log_date) AS log_date, tr_topics.ID_topic, tr_topics.topic_name FROM logs INNER JOIN (subjects INNER JOIN tr_topics ON subjects.ID_subject = tr_topics.topic_subject) ON logs.log_topicID = tr_topics.ID_topic GROUP BY logs.log_module, subjects.ID_subject, tr_topics.ID_topic, tr_topics.topic_name HAVING (logs.log_module LIKE 'training') AND (subjects.ID_subject = " + Replace(subj, "'", "''") + ") ORDER BY COUNT(logs.ID_log) DESC;"
'SQL: "SELECT COUNT(logs.ID_log) AS ID_log, MAX(logs.log_date) AS log_date, tr_topics.ID_topic, tr_topics.topic_name  FROM subjects INNER JOIN tr_topics ON subjects.ID_subject = tr_topics.topic_subject INNER JOIN logs ON tr_topics.ID_topic = logs.log_topicID  GROUP BY logs.log_module, subjects.ID_subject, tr_topics.ID_topic, tr_topics.topic_name  HAVING (logs.log_module LIKE 'training') AND (subjects.ID_subject = " + Replace(subj, "'", "''") + ")  ORDER BY COUNT(logs.ID_log) DESC;"
...
<%
call log_the_page ("Training Topics Stats: " & subj)
%>
