...
If (CStr(Request("MM_update")) <> "" And CStr(Request("MM_recordId")) <> "") Then
  ...
  MM_editRedirectUrl = "q_list_of_topics.asp?subj=" + request.querystring("subj")
...
If (CStr(Request("MM_update")) <> "" And CStr(Request("MM_recordId")) <> "") Then
  ...
  If (Not MM_abortEdit) Then
    ...
    call log_the_page ("Quiz Execute - UPDATE Topic: " & MM_recordId)
...
topics.Source = "SELECT *  FROM q_topics  WHERE ID_topic = " + Replace(topics__MMColParam, "'", "''") + ""
topics.CursorType = 0
...
<TITLE>BBP ADMIN: Quiz topic. You are logged in as <%=Session("MM_Username_admin")%></TITLE>
...
<table>
  <tr> 
    <td align="left" valign="bottom" class="heading"> Quiz topic edit</td>
  </tr>
  <tr> 
    <td align="left" valign="bottom"> 
      <form ACTION="<%=MM_editAction%>" METHOD="POST" name="add_subject" onSubmit="<%call on_form_Submit(0)%>" onReset="<%call on_form_Reset%>">
        <table>
          <tr> 
            <td width="99%" >Name of the topic</td>
          </tr>
          <tr> 
            <td width="99%" ><a href="q_list_of_topics.asp?subj="></a> 
...
%>
          ...
          <tr> 
            <td width="99%" > 
              ...
              or 
              <input type="button" name="goback" value="Go back to topic list" class="quiz_button" onClick="document.location='q_list_of_topics.asp?subj=<%=(topics.Fields.Item("topic_subject").Value)%>'">
            </td>
          </tr>
        </table>
...
<%
call log_the_page ("Quiz Edit Topic: " & (topics.Fields.Item("ID_topic").Value))
%>
