...
If (CStr(Request("MM_insert")) <> "") Then
  ...
  If (Not MM_abortEdit) Then
    ...
    call log_the_page ("BBG Execute - INSERT info4")
...
business.Source = "SELECT *  FROM q_info4"
...
function trySubmit()
{
	...
	{
		alert("Sorry, you must enter a name for a new Company!\n(min. 2 characters)");
		return false;
	}
	if (confirm("Are you sure you want to add a Company?"))	{	document.forms[0].submit();
	return false;
	}
return false;
}
...
<table>
  <tr>
    <td align="left" valign="bottom" class="heading"> Company</td>
  </tr>
  <tr>
    <td align="left" valign="bottom">
      <form name="add_bus" method="POST" action="<%=MM_editAction%>" onSubmit="<%call on_form_Submit(0)%>" onReset="<%call on_form_Reset%>">
        <table>
          <tr>
            <td colspan="2" class="subheads">Company:</td>
          </tr>
...
%>
          <tr class="table_normal" onMouseOver="pviiClassNew(this,'table_hl')" onMouseOut="pviiClassNew(this,'table_normal')">
            ...
            <td width="580" class="text"><a href="../admin/business_edit_cmp.asp?business=<%=(business.Fields.Item("id_info4").Value)%>"><%=(business.Fields.Item("info4").Value)%></a></td>
            <td width="20" class="text">
				<%if business.Fields.Item("info4_active").Value then%>
...
%>
          ...
          <tr>
            <td >&nbsp;</td>
            <td >Sorry,
              there are no Companies in the BBP currently.</td>
          </tr>
          ...
          <tr>
            ...
            <td width="99%" >
              ...
              <input type="submit" name="Submit" value="Add this Company" class="quiz_button" <%call IsEditOK%>>
            </td>
          </tr>
        </table>
...
<%
call log_the_page ("BBG List info4")
%>
