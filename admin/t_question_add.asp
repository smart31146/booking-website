...
<%
Dim monkey
If (Request.QueryString("monkey") <> "") Then 
monkey = cInt(Request.QueryString("monkey"))
Elseif (Session("monkey") <> "") Then
monkey = cInt(Session("monkey"))
else 
monkey = "0"
End If
%>
...
If (CStr(Request("MM_insert")) <> "") Then
 ...
 MM_fieldsStr  = "subject|value|topic|value|monkey|value|title|value|q_body|value|previous|value|next|value|scenario|value|foto|value|more|value|page_ord|value|active|value|UID|value"
  MM_columnsStr = "page_subject|none,none,NULL|page_topic|none,none,NULL|page_monkey|none,none,NULL|page_title|',none,''|page_text|',none,''|page_previous|none,none,NULL|page_next|none,none,NULL|page_scenario|none,none,NULL|page_photo|',none,''|page_note|',none,''|page_ord|none,none,NULL|page_active|none,1,0|page_UID|',none,''"
...
If (CStr(Request("MM_insert")) <> "") Then
  ...
  If (Not MM_abortEdit) Then
    ...
    call log_the_page ("Training Execute - INSERT Page")

session("subj") = request.form("subject")
session("topic") = request.form("topic")
session("monkey") = request.form("monkey")

set newest = Server.CreateObject("ADODB.Recordset")
newest.ActiveConnection = Connect
newest.Source = "SELECT ID_page FROM tr_pages ORDER BY ID_page DESC"
newest.CursorType = 0
newest.CursorLocation = 3
newest.LockType = 3
newest.Open()
newest_numRows = 0

  MM_editRedirectUrl = "t_question_edit.asp?qid=" & newest.Fields.Item("ID_page").Value
...
allpages_total = allpages.RecordCount

' set the number of rows displayed on this page
If (allpages_numRows < 0) Then
  allpages_numRows = allpages_total
Elseif (allpages_numRows = 0) Then
  allpages_numRows = 1
End If

' set the first and last displayed record
allpages_first = 1
allpages_last  = allpages_first + allpages_numRows - 1

' if we have the correct record count, check the other stats
If (allpages_total <> -1) Then
  If (allpages_first > allpages_total) Then allpages_first = allpages_total
  If (allpages_last > allpages_total) Then allpages_last = allpages_total
  If (allpages_numRows > allpages_total) Then allpages_numRows = allpages_total
End If
%>
<%
...
If (allpages_total = -1) Then
  ...
  While (Not allpages.EOF)
    allpages_total = allpages_total + 1
    allpages.MoveNext
  Wend
...
'  Else
  ...
  ' set the number of rows displayed on this page
  If (allpages_numRows < 0 Or allpages_numRows > allpages_total) Then
    allpages_numRows = allpages_total
  End If
...
<TITLE>BBP ADMIN: Training page add. You are logged in as <%=Session("MM_Username_admin")%></TITLE>
...
function trySubmit()
	...
	if (confirm("Are you sure you want to add a new page?"))	{	document.forms[0].submit();
	return false;
	}
return false;
	}
...
<table>
  <tr> 
    <td align="left" valign="bottom" class="headers"> Training page add </td>
  </tr>
  <tr> 
    <td align="left" valign="bottom" > 
      <form ACTION="<%=MM_editAction%>" METHOD="POST" name="editq" onSubmit="<%call on_form_Submit(0)%>" onReset="<%call on_form_Reset%>">
        <table>
...
While (NOT topics.EOF)
%>
                      <option value="<%=(topics.Fields.Item("ID_topic").Value)%>" <%if (CStr(topics.Fields.Item("ID_topic").Value) = CStr(top)) then Response.Write("SELECTED") : Response.Write("")%>><%=(topics.Fields.Item("topic_name").Value)%></option>
                      <%
  topics.MoveNext()
Wend
...
%>
                      <option value="<%=(monkeys.Fields.Item("ID_monkey").Value)%>" <%if (CStr(monkeys.Fields.Item("ID_monkey").Value) = CStr(monkey)) then Response.Write("SELECTED") : Response.Write("")%>><%=(monkeys.Fields.Item("monkey_name").Value) & ": " & (monkeys.Fields.Item("monkey_file").Value)%></option>
                      <%
  monkeys.MoveNext()
Wend
...
%>
                ...
                <tr> 
                  ...
                  <td > 
                    <input type="text" size="49" class="formitem1" onChange="change=true;" name="title">
                  </td>
                </tr>
              </table>
            </td>
          ...
          <tr align="left" valign="top"> 
            <td  colspan="2">Body <a href="javascript:" onClick="MM_openBrWindow('_editor.asp?field=q_body','editor','width=520,height=400')"><img src="images/editor.gif" width="24" height="10" border="0"></a> 
            </td>
          </tr>
          <tr align="left" valign="top"> 
            <td  colspan="2"> 
              <textarea name="q_body" rows="7" class="formitem1" onChange="change=true;" cols="100"></textarea>
            </td>
          </tr>
          <tr align="left" valign="top"> 
            <td  colspan="2"> 
...
While (NOT allpages.EOF)
%>
                      <option value="<%=(allpages.Fields.Item("ID_page").Value)%>"><%=(allpages.Fields.Item("subject_name").Value) & "-" & (allpages.Fields.Item("topic_name").Value) & ": " & (allpages.Fields.Item("page_title").Value)%></option>
                      <%
  allpages.MoveNext()
Wend
'If (allpages.CursorType > 0) Then
...
%>
					  ...
					  %>" ><%=(allpages.Fields.Item("subject_name").Value) & "-" & (allpages.Fields.Item("topic_name").Value) & ": " & (allpages.Fields.Item("page_title").Value)%></option>
                      <%
  allpages.MoveNext()
Wend
'If (allpages.CursorType > 0) Then
...
'End If
%>
                      <option value="<%=(count_pages.Fields.Item("MaxOfID_page").Value)+2%>" selected>??? 
...
%>
                ...
                <tr> 
                  ...
                  <td > 
                    <input type="text" size="70" class="formitem1" onChange="change=true;" name="foto">
                    <a href="javascript:"  onClick="MM_openBrWindow('_photo_browse.asp','photobrowse','scrollbars=yes,width=610,height=400')"><img src="images/search.gif" width="16" height="16" border="0"></a></td>
                </tr>
              </table>
            </td>
          </tr>
          <tr align="left" valign="top"> 
            <td  colspan="2">Bottom note <a href="javascript:" onClick="MM_openBrWindow('_editor.asp?field=more','editor','width=520,height=400')"><img src="images/editor.gif" width="24" height="10" border="0"></a> 
            </td>
          </tr>
          <tr align="left" valign="top"> 
            <td  colspan="2"> 
              <textarea name="more" cols="100" class="formitem1" onChange="change=true;" rows="2"></textarea>
            </td>
          </tr>
        </table>
        <p> 
          <input type="reset" name="Reset" value="Reset the form" class="quiz_button">
          <input type="submit" name="Submit" value="Insert this page" class="quiz_button" <%call IsEditOK%>>
          or 
          <input type="button" name="goback" value="Go back" class="quiz_button" onClick="history.go(-1)">
          <input type="hidden" name="page_ord" value="999999">
...
<%
call log_the_page ("Training Add a new Page: " & subj & ", " & top)
%>
