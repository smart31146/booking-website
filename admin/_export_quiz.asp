...
SQL = "SELECT * "
SQL = SQL & " FROM new_subjects s1,subjects WHERE s1.s_qiD = ID_subject  AND ABS([s_active]) = 1  ORDER BY s_qID, s_order ASC"
...
<% ELSEIF clng(obj("s_typ")) = 2 THEN
	SQL = "SELECT * FROM q_question,q_choice WHERE question_topic = "&fixstr(clng(obj("s_id")))&" AND ABS(question_active) = 1 AND ABS(choice_active) = 1 AND choice_question = ID_question"
	'response.write SQL
	question.Open SQL, Connect,3,3
