...
If (CStr(Request("MM_insert")) <> "") Then
  ...
  MM_fieldsStr  = "newfaq|value|UID|value"
  MM_columnsStr = "faq_name|',none,''|faq_UID|',none,''"
...
If (CStr(Request("MM_insert")) <> "") Then
  ...
  If (Not MM_abortEdit) Then
    ...
    call log_the_page ("BBG Execute - INSERT FAQ")	
...
Dim Repeat1__index
Repeat1__index = 0
faqs_numRows = faqs_numRows + Repeat1__numRows
%>
<HTML>
<HEAD>
<META HTTP-EQUIV="Pragma" CONTENT="no-cache">
<TITLE>BBP ADMIN: FAQ tab list. You are logged in as <%=Session("MM_Username_admin")%></TITLE>
...
function trySubmit()
{
	if (document.forms[0].newfaq.value.length<3)
	{
		alert("Sorry, you must enter a name for a new FAQ tab!\n(min. 3 characters)");
		return false;
	}
	if (confirm("Are you sure you want to add a new FAQ tab?"))	{	document.forms[0].submit();
	return false;
	}
return false;
}
...
<table>
  <tr> 
    <td align="left" valign="bottom" class="heading"> FAQ tab list</td>
  </tr>
  <tr> 
    <td align="left" valign="bottom"> 
      <form name="add_subject" method="POST" action="<%=MM_editAction%>" onSubmit="<%call on_form_Submit(0)%>" onReset="<%call on_form_Reset%>">
        <table>
          <tr> 
            <td colspan="2" class="subheads">FAQ tabs:</td>
          </tr>
          ...
          <% If Not faqs.EOF Or Not faqs.BOF Then %>
          <% 
While ((Repeat1__numRows <> 0) AND (NOT faqs.EOF)) 
%>
          <tr class="table_normal" onMouseOver="pviiClassNew(this,'table_hl')" onMouseOut="pviiClassNew(this,'table_normal')">          <td class="text" width="20"><%=numbers%></td>
            <td width="580" class="text"><a href="b_edit_faqs.asp?faq=<%=(faqs.Fields.Item("ID_faq").Value)%>"><%=(faqs.Fields.Item("faq_name").Value)%></a> </td>
          </tr>
          <% 
...
Wend
%>
          <% End If ' end Not faqs.EOF Or NOT faqs.BOF %>
          <% If faqs.EOF And faqs.BOF Then %>
          <tr> 
            <td >&nbsp;</td>
            <td >Sorry, 
              there are no FAQ tabs in the BBP currently.</td>
          </tr>
          <% End If ' end faqs.EOF And faqs.BOF %>
          <tr class="table_normal"> 
            <td ><img src="images/new2.gif" width="11" height="13"></td>
            <td width="99%" > 
              <input type="text" name="newfaq" size="85" class="formitem1">
            </td>
          </tr>
          <tr> 
            <td> 
            ...
            <td width="99%" > 
              ...
              <input type="submit" name="Submit" value="Add this new FAQ tab" class="quiz_button" <%call IsEditOK%>>
            </td>
          </tr>
        </table>
...
<%
call log_the_page ("BBG List FAQ")
%>
