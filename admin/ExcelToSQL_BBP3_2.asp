...
'Hereeeeeeeeeeeeeeeeeee Set Request_MM_insert = True when the submit button is pressed
  ...
  if err.Number = 0 then
	...
	Response.Write ("<BR> New user <B>" & login_name & "</B>  added.<BR>")
	else
	Response.Write ("<BR> DB error while adding <B>" & login_name & "</B>. Please verify details for this user and try again.<BR>")
...
Select Case Entry
	                Case "first_name"   
		               ' ReturnRequestForm = Replace(first_name, "'", "''")
					...
					Case "info1"
                     ...
                     Case "info2"
						...
						While (NOT info11.EOF)
                            if UCASE(info11.Fields.Item("info1").Value) = UCASE(user_business) then
                                id_info1 = info11.Fields.Item("ID_info1").Value
								
									set info22 = Server.CreateObject("ADODB.Recordset")
									info22.ActiveConnection = Connect
									info22.Source = "SELECT top 1 * FROM q_info2 where info2='"&user_site&"' and info2_info1 = '"&id_info1&"'"
									info22.CursorType = 0
...
sub create_user_subjects
	...
	' Made change in method to identify unique user from database
		
	new_user.Source = "SELECT * FROM q_user WHERE user_username = '" + Replace(ReturnRequestForm("login_name"), "'", "''") + "';"
...
sub create_user_subjects_exist
	...
	new_user.Source = "SELECT * FROM q_user WHERE user_username='" & Replace(CStr(login_name), "'", "''") & "'"
	'Replace(CStr(login_name), "'", "''")
	 '" + RequestSession + "';"
	new_user.CursorType = 0
	new_user.CursorLocation = 3
	new_user.LockType = 3
...
sub update_fields
				    	...
				    	Set MM_updateCmd = Server.CreateObject("ADODB.Command")
				    	MM_updateCmd.ActiveConnection = Connect
						strSql = "update q_user set user_email='"&Replace(ReturnRequestForm("email"), "'", "''")&"' where ID_user="&userID
				    	...
				    	Set MM_updateCmd = Server.CreateObject("ADODB.Command")
				    	MM_updateCmd.ActiveConnection = Connect
						strSql = "update q_user set user_reference="&user_employeereference&" where ID_user="&userID
						'strSql = "update q_user set user_email='"&ReturnRequestForm("email")&"' where ID_user="&userID
						MM_updateCmd.CommandText = strSql
					   	MM_updateCmd.Execute
					   	MM_updateCmd.ActiveConnection.Close
						...
						if err.Number = 0 then 
						Response.Write("<BR>User <B>" & (CStr(login_name)) & "</B> is already <B>exists</B> and details <B>updated</B>.<BR>")
						else
						Response.Write ("<BR> DB error while updating <B>" & login_name & "</B>. Please verify details for this user and try again.<BR>")
						End if
        ...
        While Not objRS.EOF
            ...
            if ValidUser <> 0 then
                if (FindDuplicateUserName <> "True") then
					...
					On Error GoTo 0
					Response.Write("<BR>Database error occured while adding this user<BR>")
					call FillArrayWithUserInfo()
				...
				On Error GoTo 0
				Response.Write("<BR>Database error occured while updating this user<BR>")
				create_user_subjects_exist
				update_fields
