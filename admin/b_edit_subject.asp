...
If (CStr(Request("MM_update")) <> "" And CStr(Request("MM_recordId")) <> "") Then
  ...
  MM_fieldsStr  = "subject_name|value|subject_comment|value|active|value"
  MM_columnsStr = "subject_name|',none,''|subject_comment|',none,''|subject_active_b|none,1,0"
...
If (CStr(Request("MM_update")) <> "" And CStr(Request("MM_recordId")) <> "") Then
  ...
  If (Not MM_abortEdit) Then
    ...
    call log_the_page ("BBG Execute - UPDATE Subject: " & MM_recordId)	
...
<TITLE>BBP ADMIN: Reference Subjects. You are logged in as <%=Session("MM_Username_admin")%></TITLE>
...
function trySubmit()
{
	...
	{
		alert("Sorry, you must enter a name for the subject!\n(min. 2 characters)");
		return false;
	}
...
<table>
  <tr> 
    <td align="left" valign="bottom" class="heading"> BBG subject edit</td>
  </tr>
  <tr> 
    <td align="left" valign="bottom"> 
      <form ACTION="<%=MM_editAction%>" METHOD="POST" name="add_subject" onSubmit="<%call on_form_Submit(0)%>" onReset="<%call on_form_Reset%>">
        <table>
          ...
          <tr> 
            <td width="99%" > 
              <input type="text" onChange="change=true;" name="subject_name" size="60" class="formitem1" value="<%=(subjects.Fields.Item("subject_name").Value)%>">
            </td>
          </tr>
          <tr> 
          ...
          <tr> 
            <td width="99%" > 
              <input type="text" onChange="change=true;" name="subject_comment" size="60" class="formitem1" value="<%=(subjects.Fields.Item("subject_comment").Value)%>">
            </td>
          </tr>
          <tr> 
            <td width="99%" >Subject active? 
              <input <%If (CStr(abs(subjects.Fields.Item("subject_active_b").Value)) = CStr(1)) Then Response.Write("CHECKED") : Response.Write("")%> type="checkbox" name="active" value="checkbox">
            </td>
          </tr>
          <tr> 
            <td width="99%" >&nbsp; </td>
...
<%
call log_the_page ("BBG Edit Subject: " & (subjects.Fields.Item("ID_subject").Value))
%>
