...
miserror="<br><span style='color:green' >Your password has been reset.</span><br><br><div style='font-size:14px;'> Please press OK to sign in.</div>"
        	...
        	<form name="password_form" method="post" action="reset_password.asp?focus=false">
             ...
             <input  type="password" class="radius2" name="pass_req" id="pass_req" value='<% response.write(pval)%>' />
               ...
               <button  type="submit" id="sbmt" class="radius title" value="Submit" name="submit">Submit</button>
				...
				<script>swal({   title: "",   text: "<%= miserror %>",   type: "warning",   confirmButtonText: "OK", html:true }, function (isConfirm) { if (isConfirm) {window.location.href = 'index.asp' ;} }); $("#pass_req").focus(); </script>
				<% end if %>
